...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (payload.action === "card") {
      ...
      const profile = await chatJSON(apiKey, VISION_MODEL, [
        {
          role: "system",
...
— Никогда не выдумывай год, характеристики, происхождение.`,
      ...
      const canAnchorPrice = category !== "other" && Boolean(range);
      ...
      if (desired) {
        ...
        priceInstruction = `Оценить цену этого товара автоматически нельзя — у нас нет рыночного ориентира по такой категории. В price_recommended, price_min, price_max верни 0. В price_rationale по-${lang === "kz" ? "казахски" : "русски"} мягко попроси продавца указать свою цену вручную (поле «Ваша цена»), потому что такой товар мы точно оценить не можем.`;
      }
...
${answers.freeText ? `— Рассказ продавца своими словами: ${answers.freeText}` : ""}`,
      ...
      if (desired) {
        ...
        // Оценить не смогли — честно помечаем, число не показываем.
