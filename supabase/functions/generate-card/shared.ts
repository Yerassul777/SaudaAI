...
/*
  Сколько карточек в месяц входит в бесплатный план. Считаются сохранённые
  карточки текущего календарного месяца, лимит проверяется на сервере: клиентский
  счётчик носит справочный характер и обходится за секунду.

  Тренажёр и уроки под лимит не попадают: учиться человек должен без ограничений.
*/
export const FREE_CARDS_PER_MONTH = 5;

/*
  Аккаунты, на которые лимит не действует. Здесь демо-аккаунт команды: на защите
  счётчик не должен упереться в потолок посреди выступления.

  Сверяем по техническому email, а не по номеру из user_metadata: метаданные
  пользователь может править сам через updateUser, а email завязан на вход.
  Формула email повторяет deriveEmail из src/lib/auth.ts.
*/
export const UNLIMITED_EMAILS = ["p77759181572@users.sauda-ai.kz"];
